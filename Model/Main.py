...
def load_pretrained_data(args):
    ...
    pretrain_path = '%spretrain/%s/%s.npz' % (
        args.proj_path, args.dataset, pre_model)
    try:
...
if __name__ == '__main__':
    ...
    *********************************************************
    ...
    if args.model_type == 'bprmf':
        model = BPRMF(data_config=config,
                      pretrain_data=pretrain_data, args=args)
        ...
        if ckpt and ckpt.model_checkpoint_path:
            ...
            if args.report != 1:
                ...
                ret = test(sess, model, users_to_test,
                           drop_flag=False, batch_test_flag=batch_test_flag)
                ...
                if args.save_flag == -1:
                    ...
                    #                                                  '-'.join([str(r) for r in eval(args.regs)]))
                    temp_save_path = '%spretrain/%s/%s3' % (
                        args.proj_path, args.dataset, model.model_type)
                    ensureDir(temp_save_path)
                    np.savez(temp_save_path, user_embed=user_embed,
                ...
                if args.save_flag == -2:
                    user_embed, entity_embed, relation_embed = sess.run(
                        [model.weights['user_embed'], model.weights['entity_embed'],
                            model.weights['relation_embed']],
                        feed_dict={})

                    temp_save_path = '%spretrain/%s/%s.npz' % (
                        args.proj_path, args.dataset, args.model_type)
                    ensureDir(temp_save_path)
                    np.savez(temp_save_path, user_embed=user_embed,
                             entity_embed=entity_embed, relation_embed=relation_embed)
    ...
    *********************************************************
    ...
    test_items = list(data_generator.test_user_dict.values())
